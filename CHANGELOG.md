...
Live 2026-09-02 pid 2798540 (version 3.562.0) Control #/mcp github Test filled
92 chars of JSON (`ok: true, toolCount: 2`) but left class `log placeholder`.
CSS `.log.placeholder` mutes and italicizes (same leftover as Cost 3.570.0).
Empty-state "no output yet" before click is correct. Homedir JSON store-writer
...
`runAuthCli(cfg)` at bin/xclaw.mjs:49-53 after `loadConfig()`,
auth-cli.mjs:123) already had cfg in scope. Two xclaw instances on one
host with different `paths.configDir` shared one cookie-rotation.json,
so instance B restored instance A's rotation generation / use-count.
The suite wrote the operator's real `~/.xclaw`.

Honour `cfg.auth?.web?.rotationStatePath` / `previousSessionPath` then
`paths.configDir` then `XCLAW_CONFIG_DIR` then null. No home fallback.
Do not honour `XCLAW_STATE_DIR`. No new env. Keep `XCLAW_COOKIE_ROTATION`
as the strategy env (not a path). `writeState` still no-ops without
persisting (do not `mkdir(null)`). `readState` returns the empty
default. `rotateWebSession` skips the archive branch when
`previousPath` is null. Removed `os` import (only used by the leftover
home fallback). Production writers always have configDir, so live
persist is not dropped.
...
missions/engine.mjs and self/deploy.mjs:169) already had cfg in
scope. `loadConfig()` stamps `paths.configDir` unconditionally, so
the resolver still homed via the leftover `os.homedir()` fallback.
Two instances on one host with different `paths.configDir` shared
one missions/ directory; the suite wrote the operator's real
`~/.xclaw`. Same class as v3.297.0 `alert-state.json` and
v3.535.0 `skill-proposals/`. Honour existing `XCLAW_CONFIG_DIR`.

- Honour `paths.configDir` then `XCLAW_CONFIG_DIR` then null. No home
  fallback. Do not honour `XCLAW_STATE_DIR`. No new env.
- `saveMission` still returns the in-memory mission without persisting
  (do not `mkdir(null)`). `listMissions` returns `[]`. `loadMission`
  returns `null`. Production writers always have configDir, so live
  persist is not dropped.
- Pin: configDir write never touches home; no-configDir never writes
  home or cwd/`null`; `XCLAW_CONFIG_DIR` still wins when no configDir.
...
different `paths.configDir` shared one skill-proposals/ directory;
the suite wrote the operator's real `~/.xclaw`. Same class as
v3.297.0 `alert-state.json` and v3.534.0 `soak/`. Honour existing
`XCLAW_CONFIG_DIR`.

- Honour `paths.configDir` then `XCLAW_CONFIG_DIR` then null. No home
  fallback. Do not honour `XCLAW_STATE_DIR`. No new env.
- `proposeSkillFromFailure` / `proposeSkillFromSuccess` still return
  the in-memory draft without persisting (do not `mkdir(null)`).
  `listProposals` returns `[]`. Keep honouring `paths.skillsDir` for
  install dest. Production writers always have configDir, so live
  persist is not dropped.
- Pin: configDir write never touches home; no-configDir never writes
  home or cwd/`null`; `XCLAW_CONFIG_DIR` still wins when no configDir.
...
resolver still homed via the leftover `os.homedir()` fallback. Two
instances on one host with different `paths.configDir` shared one
vault; the suite wrote the operator's real `~/.xclaw`. Same class as
v3.297.0 `alert-state.json` and v3.523.0 `connected-tokens.json`.
Honour existing `XCLAW_CONFIG_DIR`.

- Honour `paths.configDir` then `XCLAW_CONFIG_DIR` then null. No home
  fallback. Do not honour `XCLAW_STATE_DIR`. No new env.
- `vaultSave` no-ops a null path (do not `mkdir(null)` /
  `path.dirname(null)` / rename `"null.bak-*"`). `vaultLoad` returns
  `{ version: 1, apps: {}, userId }`. Production writers always have
  configDir, so live persist is not dropped.
- Pin: configDir write never touches home; no-configDir never writes
  home or cwd/`null`; `XCLAW_CONFIG_DIR` still wins when no configDir.
...
`reserveUsd(cfg)` at swarm-ledger / job.mjs) and doctor
(`readLease(cfg)`) already had cfg in scope. `loadConfig()` stamps
`paths.configDir` unconditionally, so the resolver still homed via
the leftover `os.homedir()` fallback. Two instances on one host with
different `paths.configDir` shared one lease file; the suite wrote
the operator's real `~/.xclaw`. Same class as v3.297.0
`alert-state.json` and v3.519.0 `swarm-cost-ledger.json`. Honour
existing `XCLAW_CONFIG_DIR`.

- Honour `paths.configDir` then `XCLAW_CONFIG_DIR` then null. No home
  fallback. Do not honour `XCLAW_STATE_DIR`. No new env.
- `acquireLease` / `renewLease` no-op a null path (`ok: true`,
  `skipped: true`; do not `mkdir(null)` / `path.dirname(null)`).
  `releaseLease` returns absent. `readLease` returns null.
  Production callers always have configDir, so live leasing is not
  dropped.
- Pin: configDir write never touches home; no-configDir never writes
  home or cwd/`null`; `XCLAW_CONFIG_DIR` still wins when no configDir.
...
### Swarm ledger follows paths.configDir

`ledgerPath()` resolved `~/.xclaw/swarm-cost-ledger.json` from
`os.homedir()` while production jobs (`reserveUsd(cfg)` /
`settleUsd(cfg)` at job.mjs), doctor, stop-health, and eval smoke
already had cfg in scope. `loadConfig()` stamps `paths.configDir`
unconditionally, so the resolver still homed via the leftover
`os.homedir()` fallback. Two instances on one host with different
`paths.configDir` shared one daily swarm cap; the suite wrote the
operator's real `~/.xclaw`. Same class as v3.297.0 `alert-state.json`
and v3.518.0 `accounts/`. Honour existing `XCLAW_CONFIG_DIR`.

- Honour `paths.configDir` then `XCLAW_CONFIG_DIR` then null. No home
  fallback. Do not honour `XCLAW_STATE_DIR`. No new env.
- `save()` no-ops a null path (do not `mkdir(null)` /
  `path.dirname(null)`). `load()` returns empty. In-memory
  reserve/settle does not persist. Production callers always have
  configDir, so live persistence is not dropped.
- Pin: configDir write never touches home; no-configDir never writes
  home or cwd/`null`; `XCLAW_CONFIG_DIR` still wins when no configDir.
...
Webchat, `processInbound`, voice `/ws/voice`, the TUI, Discord `/ask`, and
the CLI already call `autoPromoteIfNeeded` when `stopReason === "maxTurns"`,
so a truncated turn becomes a durable objective. Voice TUI `preferAgent`
called `runJob` then printed/spoke the truncated reply — a cutoff never
became a mission. Distinct from closed `/ws/voice`.

The preferAgent path now promotes the same way the TUI does (detached
notify into the transcript, `formatPromotedReply` before print/speak).
`runJob` already persists by default — this slice does not mint
`persistRun: true` and does not auto-promote HTTP `POST /agent/run`
(caller-owned `stopReason`). Voice listen is a sibling, not this slice.
...
Telegram and Discord MESSAGE already pass `notify` into `processInbound`,
so `shouldAutoPromoteTurn` can fire on `stopReason === "maxTurns"`. Slack
went through the same helper but omitted `notify`, so a truncated Slack
turn never became a mission.

`handleMessage` now passes `notify` (detached `sendMessage` into the
originating thread). Named `chatId` already persists — this slice does
not mint `persistRun: true` and does not auto-promote HTTP
`POST /agent/run` (caller-owned `stopReason`). Email notify is a sibling
gap, not this slice.
...
Bearer `x_keyword_search` still called `res.json()` before `res.ok`.
A 429 HTML body threw `Unexpected token` instead of `HTTP 429`.
json() is now catch-empty, matching user/thread fetch.
