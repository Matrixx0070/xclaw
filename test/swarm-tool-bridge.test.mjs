...
import {
  ...
  DEFAULT_ALLOW,
} from "../src/swarm/tool-bridge.mjs";
...
test("defaults expose swarmExt.tools fail-closed knobs", () => {
  assert.equal(DEFAULT_CONFIG.swarm.decompose.tools.enabled, true);
  assert.equal(DEFAULT_CONFIG.swarm.decompose.tools.autoApproveMaxTier, "low");
});
...
test("operator can raise the tier ceiling via config", async () => {
  const f = fakes();
  const cfg = { swarm: { decompose: { tools: { autoApproveMaxTier: "risky" } } } };
