...
export async function createXclawToolBridge(cfg = {}, overrides = {}) {
  const toolsCfg = cfg.swarm?.decompose?.tools ?? cfg.swarmExt?.tools ?? {};
  const allow = new Set(
  ...
  // Optional operator egress policy (ADR 0004 — wires the formerly-dead
  // ToolPolicy engine): cfg …tools.policy = { egress: "allow"|"deny"|
  // "allowlist", allowlist: [tool names], blocklist: [tool names],
  // urlAllowlist via `allowlist` host entries }. Absent → allow-all
  // (risk gate still applies).
  let toolPolicy = null;
  if (toolsCfg.policy && typeof toolsCfg.policy === "object") {
    const { ToolPolicy } = await import("./decompose/tool-policy.mjs");
    toolPolicy = new ToolPolicy(toolsCfg.policy);
  }
  ...
  async function execute(name, params) {
    ...
    // Operator egress policy (before the risk gate; applies to EVERY tool
    // including alwaysAllow — the operator's deny beats the research bypass).
    if (toolPolicy) {
      const pol = toolPolicy.canExecute(name, args);
      if (!pol.allowed) {
        return {
          success: false,
          blocked: true,
          error: `blocked by swarm egress policy: ${pol.reason}`,
        };
      }
    }
