...
async function ensureDirsAndFile() {
  ...
  try {
    ...
    console.log(
      `[xclaw] First-run tip: profile="${cfg.profile || "lab"}" (lab=auto-approve tools). For locked-down use: XCLAW_PROFILE=prod and set XCLAW_GATEWAY_TOKEN. /link only works in DMs.`
    );
  }
}
