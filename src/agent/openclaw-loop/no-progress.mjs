/**
 * Adapted from OpenClaw (MIT) — src/agents/tool-loop-no-progress.ts
 */
export function getNoProgressStreak(history, toolName, argsHash) {
  const repeatedArguments = countNoProgressStreak(history, toolName, argsHash, false);
  if (toolName !== "exec" && toolName !== "xclaw_bash" && toolName !== "bash") {
    return repeatedArguments;
  }
  const terminalFailures = countNoProgressStreak(history, toolName, argsHash, true);
  return terminalFailures.count > repeatedArguments.count
    ? terminalFailures
    : repeatedArguments;
}
