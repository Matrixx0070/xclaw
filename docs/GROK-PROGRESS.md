...
## 2026-09-01 — 3.514.0 Control-plane sqlite follows paths.configDir

STATUS: green (local hermetic pin)
DISCOVERED: JSON.parse / ENOENT / doctorHorizon({}) hunt exhausted (class 56/57). Load-await-write token-store remainder exhausted 3.500–3.504. Gateway request-path unbounded fetch exhausted 3.505.0. PagerDuty webhook history closed 3.506.0. Pairing store closed 3.507.0. Sessions persist closed 3.508.0. Usage-tracker ledger closed 3.509.0. Compaction offload closed 3.510.0. Telegram writer lock closed 3.511.0. Gateway supervised closed 3.512.0. Automations store closed 3.513.0. Named remainder from source: `src/state/control-plane.mjs` `controlPlaneFile()` still `os.homedir()` while production `getControlPlane(cfg)` at gateway boot already had cfg in scope. `loadConfig()` stamps `paths.configDir` unconditionally and does not stamp `stateDir` or `controlPlaneFile`, so the resolver still homed. Two instances shared one control.sqlite; the suite wrote the operator's real `~/.xclaw/state/control.sqlite`. Same class as v3.297.0 `alert-state.json`. Honour existing `XCLAW_CONTROL_PLANE_FILE`. Do not honour `XCLAW_STATE_DIR` (seats/auth fallback). Closed siblings (do not re-fix): automations (3.513.0), gateway supervised (3.512.0), telegram lock (3.511.0), compaction (3.510.0), usage-tracker (3.509.0), sessions/persist (3.508.0), pairing-store (3.507.0), pagerduty-webhooks (3.506.0), alerts.mjs, cron/eval-job, cron/doctor-job, cron/live-e2e-job (already cronLogPath), model-stats.mjs (already configDir), transcript (already configDir first), auth siblings (already configDir first). `cua-retry-metrics.mjs` `metricsPath()` is NOT this class (no cfg at live callers). `memoryIndexFile` / `agentStoreFile` are specialized-dir siblings, not this slice.
BUILT: `controlPlaneFile(cfg)` honours `paths.controlPlaneFile` then `XCLAW_CONTROL_PLANE_FILE` then `paths.stateDir` then `paths.configDir` then null. No home fallback. `openControlPlane` / exclusive / `getControlPlane` no-op a null path (do not `mkdir(null)`). Doctor `probeSqlFile` and `existsSync` skip a null path. `doctor --fix` skips absorb when there is no plane. Production already threads cfg so live still persists under configDir (never drop the capability). Pin: configDir write never touches home; explicit controlPlaneFile wins; stateDir wins over configDir; env wins over configDir; no-configDir names no file and never writes home; gateway still calls `getControlPlane(cfg)`.
RAN: node --test test/control-plane-config-dir.test.mjs test/control-plane.test.mjs test/sql-quarantine.test.mjs test/doctor-fix.test.mjs → # tests 47 # pass 47 # fail 0 # duration_ms 1968.571319; npm test (hermetic) → # tests 5191 # pass 5191 # fail 0 # duration_ms 82500.547629
UNVERIFIED: GitHub `ci` on this SHA; live two-instance configDir isolation on a running gateway; live gateway restart leftover stay-put.
NEXT: remaining non-S3 evolution gap from live source. Specialized-dir SQLite siblings `memoryIndexFile` / `agentStoreFile` remain. Do not invert default-path durability. Do not rebuild S3 listed callers. Do not mint persistRun:true on voice, TUI stream body, Discord `/ask`, Slack, email, voice TUI, or voice listen. Do not auto-promote HTTP POST /agent/run. Do not stamp `runs resume --gateway` before GET `/health`.

## 2026-09-01 — 3.515.0 MITM confdir follows paths.configDir

STATUS: green (local hermetic pin)
DISCOVERED: JSON.parse / ENOENT / doctorHorizon({}) hunt exhausted (class 56/57). Load-await-write token-store remainder exhausted 3.500–3.504. Gateway request-path unbounded fetch exhausted 3.505.0. PagerDuty webhook history closed 3.506.0. Pairing store closed 3.507.0. Sessions persist closed 3.508.0. Usage-tracker ledger closed 3.509.0. Compaction offload closed 3.510.0. Telegram writer lock closed 3.511.0. Gateway supervised closed 3.512.0. Automations store closed 3.513.0. Control-plane sqlite closed 3.514.0. Named remainder from source: `src/browser/mitm.mjs` `mitmConfdir()` still `os.homedir()` while production `startMitm(cfg)` at supervisor and browser-tools already had cfg in scope. `loadConfig()` stamps `paths.configDir` unconditionally and does not stamp `browser.mitm.confdir`, so the resolver still homed. Supervisor `loadMitmCfg()` is raw JSON without configDir, and `tick()` rotated `flows.jsonl` via `mitmConfdir()` with no cfg. Two instances shared one mitm confdir (pid/log/flows/CA); the suite wrote the operator's real `~/.xclaw/mitm`. Same class as v3.297.0 `alert-state.json`. Honour existing `XCLAW_MITM_CONFDIR`. Do not honour `XCLAW_STATE_DIR` (seats/auth fallback). Closed siblings (do not re-fix): control-plane (3.514.0), automations (3.513.0), gateway supervised (3.512.0), telegram lock (3.511.0), compaction (3.510.0), usage-tracker (3.509.0), sessions/persist (3.508.0), pairing-store (3.507.0), pagerduty-webhooks (3.506.0), alerts.mjs, cron/eval-job, cron/doctor-job, cron/live-e2e-job (already cronLogPath), model-stats.mjs (already configDir), transcript (already configDir first), auth siblings (already configDir first). `cua-retry-metrics.mjs` `metricsPath()` is NOT this class (no cfg at live callers). `memoryIndexFile` / `agentStoreFile` have no production writer — not this slice. `src/swarm/tool-bridge.mjs` homes a workspace `workingDir`, not a durable store of this class.
BUILT: `mitmConfdir(cfg)` honours `XCLAW_MITM_CONFDIR` then `browser.mitm.confdir` then `paths.configDir` then null. No home fallback. `ensureMitmConfdir` / `startMitm` no-op a null path (do not `mkdir(null)`). `stopMitm` / `clearMitmFlows` / `mitmStatus` / `readMitmFlows` skip a null confdir. Supervisor stamps `paths.configDir` onto `loadMitmCfg` and threads cfg into `tick` so live still persists under configDir when MITM is on (never drop the capability). Maintenance proofs prune skips a null confdir. Pin: configDir write never touches home; explicit confdir wins; env wins over configDir; no-configDir names no dir and never writes home; supervisor still stamps configDir and tick threads cfg.
RAN: node --test test/mitm-confdir-config-dir.test.mjs test/browser-mitm.test.mjs test/browser-truth-cfg.test.mjs → # tests 40 # pass 40 # fail 0 # duration_ms 603.508789; npm test (hermetic) → # tests 5198 # pass 5198 # fail 0 # duration_ms 88541.914263
UNVERIFIED: GitHub `ci` on this SHA; live two-instance configDir isolation on a running gateway with MITM on; live supervisor tick rotate of flows.jsonl under a non-home configDir.
NEXT: remaining non-S3 evolution gap from live source. Specialized-dir SQLite siblings `memoryIndexFile` / `agentStoreFile` have no production writer. Do not invert default-path durability. Do not rebuild S3 listed callers. Do not mint persistRun:true on voice, TUI stream body, Discord `/ask`, Slack, email, voice TUI, or voice listen. Do not auto-promote HTTP POST /agent/run. Do not stamp `runs resume --gateway` before GET `/health`.

## 2026-09-01 — 3.516.0 Truth/sense no-op a null mitm confdir

STATUS: green (local hermetic pin)
DISCOVERED: JSON.parse / ENOENT / doctorHorizon({}) hunt exhausted (class 56/57). Load-await-write token-store remainder exhausted 3.500–3.504. Gateway request-path unbounded fetch exhausted 3.505.0. PagerDuty webhook history closed 3.506.0. Pairing store closed 3.507.0. Sessions persist closed 3.508.0. Usage-tracker ledger closed 3.509.0. Compaction offload closed 3.510.0. Telegram writer lock closed 3.511.0. Gateway supervised closed 3.512.0. Automations store closed 3.513.0. Control-plane sqlite closed 3.514.0. MITM confdir closed 3.515.0. Named leftover of 3.515.0 from source: `savePolicy` / `exportProofBundle` / `bindActionFlows` / `readActionBindings` still `fs.mkdir(confdir)` / `path.join(confdir, ...)` after `mitmConfdir()` became nullable. Node `path.join(null, "proofs")` becomes `"null/proofs"` in cwd. Loop `afterBrowserToolTruth(name, result)` dropped cfg even though `runAgentLoop` has it in scope; hooks afterAction already threaded cfg into bindActionFlows but dropped it into afterBrowserToolTruth. Same trap as 3.514.0 control-plane mkdir(null) and 3.515.0 `ensureMitmConfdir`. Production already threads cfg into savePolicy / exportProofBundle / bindActionFlows. Do not reopen the 3.515.0 resolver. Do not honour `XCLAW_STATE_DIR`. Closed siblings (do not re-fix): MITM confdir (3.515.0), control-plane (3.514.0), automations (3.513.0), gateway supervised (3.512.0), telegram lock (3.511.0), compaction (3.510.0), usage-tracker (3.509.0), sessions/persist (3.508.0), pairing-store (3.507.0), pagerduty-webhooks (3.506.0), alerts.mjs, cron/eval-job, cron/doctor-job, cron/live-e2e-job (already cronLogPath), model-stats.mjs (already configDir), transcript (already configDir first), auth siblings (already configDir first). `cua-retry-metrics.mjs` `metricsPath()` is NOT this class (no cfg at live callers). `memoryIndexFile` / `agentStoreFile` have no production writer — not this slice.
BUILT: `savePolicy` / `exportProofBundle` no-op a null confdir (`ok: false`, `code: MITM_NO_CONFDIR`, do not `mkdir(null)`). Explicit `opts.dest` still writes. `loadPolicy` returns emptyPolicy without join. `bindActionFlows` still returns the in-memory rec; disk append skips. `readActionBindings` returns `[]`. Loop and hooks thread cfg into `afterBrowserToolTruth` so live still loads policy under configDir when truth-auto is on (never drop the capability). `mitm_export` tool returns errorResult on `ok: false` instead of interpolating a missing path. Pin: configDir write never touches home; no-configDir never writes home or cwd/`null`; explicit dest still writes; loop/hooks still pass cfg.
RAN: node --test test/truth-sense-null-confdir.test.mjs test/browser-truth.test.mjs test/browser-truth-cfg.test.mjs test/browser-sense.test.mjs test/mitm-confdir-config-dir.test.mjs test/browser-mitm.test.mjs → # tests 59 # pass 59 # fail 0 # duration_ms 410.366909; npm test (hermetic) → # tests 5201 # pass 5201 # fail 0 # duration_ms 93369.214111
UNVERIFIED: GitHub `ci` on this SHA; live two-instance configDir isolation on a running gateway with MITM/truth-auto on; live mitm_policy set / mitm_export under a non-home configDir.
NEXT: remaining non-S3 evolution gap from live source. Specialized-dir SQLite siblings `memoryIndexFile` / `agentStoreFile` have no production writer. Do not invert default-path durability. Do not rebuild S3 listed callers. Do not mint persistRun:true on voice, TUI stream body, Discord `/ask`, Slack, email, voice TUI, or voice listen. Do not auto-promote HTTP POST /agent/run. Do not stamp `runs resume --gateway` before GET `/health`.

## 2026-09-01 — 3.517.0 Cost governor follows paths.configDir

STATUS: green (local hermetic pin)
DISCOVERED: JSON.parse / ENOENT / doctorHorizon({}) hunt exhausted (class 56/57). Load-await-write token-store remainder exhausted 3.500–3.504. Gateway request-path unbounded fetch exhausted 3.505.0. PagerDuty webhook history closed 3.506.0. Pairing store closed 3.507.0. Sessions persist closed 3.508.0. Usage-tracker ledger closed 3.509.0. Compaction offload closed 3.510.0. Telegram writer lock closed 3.511.0. Gateway supervised closed 3.512.0. Automations store closed 3.513.0. Control-plane sqlite closed 3.514.0. MITM confdir closed 3.515.0. Truth/sense null-confdir leftover closed 3.516.0. Named remainder from source: `src/tokens/cost-governor.mjs` `ledgerPath()` resolved `~/.xclaw/cost-governor.json` from `os.homedir()` while production loop (`recordJobCost(cfg)` at loop.mjs:2096), queue (`getCostGovernorStatus(worker.cfg)`), doctor, tokens routes (`GET /cost` / `POST /cost/pause`), and role-router already had cfg in scope. `loadConfig()` stamps `paths.configDir` unconditionally and does not stamp `cost.governorPath`, so the resolver still homed. Two instances on one host with different `paths.configDir` shared one daily spend/pause latch; the suite wrote the operator's real `~/.xclaw`. Same class as v3.297.0 `alert-state.json` and v3.509.0 `cost-ledger.jsonl`. `loop-cost-record.test.mjs` HOME-overrode because of this — that override is evidence of the leak, not a fix. Do not honour `XCLAW_STATE_DIR`. No new env. Closed siblings (do not re-fix): truth/sense null-confdir (3.516.0), MITM confdir (3.515.0), control-plane (3.514.0), automations (3.513.0), gateway supervised (3.512.0), telegram lock (3.511.0), compaction (3.510.0), usage-tracker (3.509.0), sessions/persist (3.508.0), pairing-store (3.507.0), pagerduty-webhooks (3.506.0), alerts.mjs, cron/eval-job, cron/doctor-job, cron/live-e2e-job, model-stats.mjs, transcript, auth siblings. `cua-retry-metrics.mjs` `metricsPath()` is NOT this class (no cfg at live callers). `memoryIndexFile` / `agentStoreFile` have no production writer. Fabric `fabricDir()` / `fabricRoot()` take NO cfg — not this class. `account-links.mjs` `accountsDir()` is a sibling — not this slice.
BUILT: `governorLedgerPath(cfg)` honours `cost.governorPath` then `paths.configDir` then null. No home fallback. `saveLedger` / `withLedgerLock` no-op a null path (do not `mkdir(null)` / `"null.lock"`). `loadLedger` returns emptyLedger. In-memory spend/pause does not persist. Production callers always have configDir, so live persistence is not dropped. Pin: configDir write never touches home; no-configDir never writes home or cwd/`null`; explicit `cost.governorPath` still wins.
RAN: node --test test/cost-governor-config-dir.test.mjs test/cost-band-transitions.test.mjs test/cost-governor-atomic.test.mjs test/cost-governor-day-race.test.mjs test/cost-governor-rollover.test.mjs test/cost-governor-route.test.mjs test/cost-soft-cap.test.mjs test/cost-estimation.test.mjs test/cron-job-cfg.test.mjs test/economy-routing.test.mjs test/loop-cost-record.test.mjs test/queue-governor-release.test.mjs → # tests 51 # pass 51 # fail 0 # duration_ms 2918.276977; npm test (hermetic) → # tests 5206 # pass 5206 # fail 0 # duration_ms 90002.091815
UNVERIFIED: GitHub `ci` on this SHA; live two-instance configDir isolation on a running gateway; live `/cost/pause` under a non-home configDir.
NEXT: remaining non-S3 evolution gap from live source. `account-links.mjs` `accountsDir()` is a sibling. Do not invert default-path durability. Do not rebuild S3 listed callers. Do not mint persistRun:true on voice, TUI stream body, Discord `/ask`, Slack, email, voice TUI, or voice listen. Do not auto-promote HTTP POST /agent/run. Do not stamp `runs resume --gateway` before GET `/health`.

## 2026-09-01 — 3.518.0 Account links follow paths.configDir

STATUS: green (local hermetic pin)
DISCOVERED: JSON.parse / ENOENT / doctorHorizon({}) hunt exhausted (class 56/57). Load-await-write token-store remainder exhausted 3.500–3.504. Gateway request-path unbounded fetch exhausted 3.505.0. PagerDuty webhook history closed 3.506.0. Pairing store closed 3.507.0. Sessions persist closed 3.508.0. Usage-tracker ledger closed 3.509.0. Compaction offload closed 3.510.0. Telegram writer lock closed 3.511.0. Gateway supervised closed 3.512.0. Automations store closed 3.513.0. Control-plane sqlite closed 3.514.0. MITM confdir closed 3.515.0. Truth/sense null-confdir leftover closed 3.516.0. Cost governor closed 3.517.0. Named remainder from source: `src/connected/account-links.mjs` `accountsDir()` resolved `~/.xclaw/accounts` from `os.homedir()` while production channel commands (`createPairingCode(cfg)` / `consumePairingCode(cfg)` / `unlinkIdentity(cfg)`), auth-legacy CLI, and doctor already had cfg in scope. `loadConfig()` stamps `paths.configDir` unconditionally and does not stamp `paths.accountsDir`, so the resolver still homed. Two instances on one host with different `paths.configDir` shared one `links.json` / `pairing.json`; the suite wrote the operator's real `~/.xclaw/accounts`. Same class as v3.297.0 `alert-state.json` and v3.517.0 `cost-governor.json`. Do not honour `XCLAW_STATE_DIR`. No new env. Closed siblings (do not re-fix): cost-governor (3.517.0), truth/sense null-confdir (3.516.0), MITM confdir (3.515.0), control-plane (3.514.0), automations (3.513.0), gateway supervised (3.512.0), telegram lock (3.511.0), compaction (3.510.0), usage-tracker (3.509.0), sessions/persist (3.508.0), pairing-store (3.507.0), pagerduty-webhooks (3.506.0), alerts.mjs, cron/eval-job, cron/doctor-job, cron/live-e2e-job, model-stats.mjs, transcript, auth siblings. `cua-retry-metrics.mjs` `metricsPath()` is NOT this class (no cfg at live callers). `memoryIndexFile` / `agentStoreFile` have no production writer. Fabric `fabricDir()` / `fabricRoot()` take NO cfg — not this class. `swarm-ledger.mjs` `ledgerPath()` is a sibling — not this slice.
BUILT: `accountsDir(cfg)` honours `paths.accountsDir` then `paths.configDir` then null. No home fallback. `saveAccountStore` / `savePairing` no-op a null path (do not `mkdir(null)` / `path.join(null)`). `loadAccountStore` / `loadPairing` return empty. In-memory create/link/pair does not persist. Production callers always have configDir, so live persistence is not dropped. Pin: configDir write never touches home; no-configDir never writes home or cwd/`null`; explicit `paths.accountsDir` still wins.
RAN: node --test test/account-links-config-dir.test.mjs test/account-links.test.mjs test/account-pairing.test.mjs → # tests 12 # pass 12 # fail 0 # duration_ms 322.356255; npm test (hermetic) → # tests 5211 # pass 5211 # fail 0 # duration_ms 90665.080271
UNVERIFIED: GitHub `ci` on this SHA; live two-instance configDir isolation on a running gateway; live `/link` under a non-home configDir.
NEXT: remaining non-S3 evolution gap from live source. `swarm-ledger.mjs` `ledgerPath()` is a sibling. Do not invert default-path durability. Do not rebuild S3 listed callers. Do not mint persistRun:true on voice, TUI stream body, Discord `/ask`, Slack, email, voice TUI, or voice listen. Do not auto-promote HTTP POST /agent/run. Do not stamp `runs resume --gateway` before GET `/health`.

## 2026-09-01 — 3.519.0 Swarm ledger follows paths.configDir

STATUS: green (local hermetic pin)
DISCOVERED: JSON.parse / ENOENT / doctorHorizon({}) hunt exhausted (class 56/57). Load-await-write token-store remainder exhausted 3.500–3.504. Gateway request-path unbounded fetch exhausted 3.505.0. PagerDuty webhook history closed 3.506.0. Pairing store closed 3.507.0. Sessions persist closed 3.508.0. Usage-tracker ledger closed 3.509.0. Compaction offload closed 3.510.0. Telegram writer lock closed 3.511.0. Gateway supervised closed 3.512.0. Automations store closed 3.513.0. Control-plane sqlite closed 3.514.0. MITM confdir closed 3.515.0. Truth/sense null-confdir leftover closed 3.516.0. Cost governor closed 3.517.0. Account links closed 3.518.0. Named remainder from source: `src/tokens/swarm-ledger.mjs` `ledgerPath()` resolved `~/.xclaw/swarm-cost-ledger.json` from `os.homedir()` while production jobs (`reserveUsd(cfg)` / `settleUsd(cfg)` at job.mjs), doctor, stop-health, and eval smoke already had cfg in scope. `loadConfig()` stamps `paths.configDir` unconditionally, so the resolver still homed via the leftover `os.homedir()` fallback. Two instances on one host with different `paths.configDir` shared one daily swarm cap; the suite wrote the operator's real `~/.xclaw`. Same class as v3.297.0 `alert-state.json` and v3.518.0 `accounts/`. Honour existing `XCLAW_CONFIG_DIR`. Do not honour `XCLAW_STATE_DIR`. No new env. Closed siblings (do not re-fix): account-links (3.518.0), cost-governor (3.517.0), truth/sense null-confdir (3.516.0), MITM confdir (3.515.0), control-plane (3.514.0), automations (3.513.0), gateway supervised (3.512.0), telegram lock (3.511.0), compaction (3.510.0), usage-tracker (3.509.0), sessions/persist (3.508.0), pairing-store (3.507.0), pagerduty-webhooks (3.506.0), alerts.mjs, cron/eval-job, cron/doctor-job, cron/live-e2e-job, model-stats.mjs, transcript, auth siblings. `cua-retry-metrics.mjs` `metricsPath()` is NOT this class (no cfg at live callers). `memoryIndexFile` / `agentStoreFile` have no production writer. Fabric `fabricDir()` / `fabricRoot()` take NO cfg — not this class. `ledger-lease.mjs` `leasePath()` is a sibling — not this slice.
BUILT: `ledgerPath(cfg)` honours `paths.configDir` then `XCLAW_CONFIG_DIR` then null. No home fallback. `save()` no-ops a null path (do not `mkdir(null)` / `path.dirname(null)`). `load()` returns empty. In-memory reserve/settle does not persist. Production callers always have configDir, so live persistence is not dropped. Pin: configDir write never touches home; no-configDir never writes home or cwd/`null`; `XCLAW_CONFIG_DIR` still wins when no configDir.
RAN: node --test test/swarm-ledger-config-dir.test.mjs test/swarm-ledger.test.mjs test/swarm-ledger-hardcap.test.mjs test/swarm-ledger-lease-gate.test.mjs test/swarm-ledger-zero-cap.test.mjs test/swarm-job-deny.test.mjs test/swarm-job-deny-integration.test.mjs test/swarm-job-deny-contract.test.mjs test/doctor-ledger-wired.test.mjs test/swarm-offline-smoke.test.mjs → # tests 33 # pass 33 # fail 0 # duration_ms 331.172277; npm test (hermetic) → # tests 5216 # pass 5216 # fail 0 # duration_ms 89291.349252
UNVERIFIED: GitHub `ci` on this SHA; live two-instance configDir isolation on a running gateway; live swarm reserve/settle under a non-home configDir.
NEXT: remaining non-S3 evolution gap from live source. `ledger-lease.mjs` `leasePath()` is a sibling. Do not invert default-path durability. Do not rebuild S3 listed callers. Do not mint persistRun:true on voice, TUI stream body, Discord `/ask`, Slack, email, voice TUI, or voice listen. Do not auto-promote HTTP POST /agent/run. Do not stamp `runs resume --gateway` before GET `/health`.

## 2026-09-01 — 3.520.0 Ledger lease follows paths.configDir

STATUS: green (local hermetic pin)
DISCOVERED: JSON.parse / ENOENT / doctorHorizon({}) hunt exhausted (class 56/57). Load-await-write token-store remainder exhausted 3.500–3.504. Gateway request-path unbounded fetch exhausted 3.505.0. PagerDuty webhook history closed 3.506.0. Pairing store closed 3.507.0. Sessions persist closed 3.508.0. Usage-tracker ledger closed 3.509.0. Compaction offload closed 3.510.0. Telegram writer lock closed 3.511.0. Gateway supervised closed 3.512.0. Automations store closed 3.513.0. Control-plane sqlite closed 3.514.0. MITM confdir closed 3.515.0. Truth/sense null-confdir leftover closed 3.516.0. Cost governor closed 3.517.0. Account links closed 3.518.0. Swarm ledger closed 3.519.0. Named remainder from source: `src/tokens/ledger-lease.mjs` `leasePath()` resolved `~/.xclaw/swarm-ledger.lease` from `os.homedir()` while production reserve (`acquireLease(cfg)` from `reserveUsd(cfg)` at swarm-ledger / job.mjs) and doctor (`readLease(cfg)`) already had cfg in scope. `loadConfig()` stamps `paths.configDir` unconditionally, so the resolver still homed via the leftover `os.homedir()` fallback. Two instances on one host with different `paths.configDir` shared one lease file; the suite wrote the operator's real `~/.xclaw`. Same class as v3.297.0 `alert-state.json` and v3.519.0 `swarm-cost-ledger.json`. Honour existing `XCLAW_CONFIG_DIR`. Do not honour `XCLAW_STATE_DIR`. No new env. Closed siblings (do not re-fix): swarm-ledger (3.519.0), account-links (3.518.0), cost-governor (3.517.0), truth/sense null-confdir (3.516.0), MITM confdir (3.515.0), control-plane (3.514.0), automations (3.513.0), gateway supervised (3.512.0), telegram lock (3.511.0), compaction (3.510.0), usage-tracker (3.509.0), sessions/persist (3.508.0), pairing-store (3.507.0), pagerduty-webhooks (3.506.0), alerts.mjs, cron/eval-job, cron/doctor-job, cron/live-e2e-job, model-stats.mjs, transcript, auth siblings. `cua-retry-metrics.mjs` `metricsPath()` is NOT this class (no cfg at live callers). `memoryIndexFile` / `agentStoreFile` have no production writer. Fabric `fabricDir()` / `fabricRoot()` take NO cfg — not this class. `ledger-lease-redis.mjs` is a Redis adapter skeleton — not this slice.
BUILT: `leasePath(cfg)` honours `paths.configDir` then `XCLAW_CONFIG_DIR` then null. No home fallback. `acquireLease` / `renewLease` no-op a null path (`ok: true`, `skipped: true`; do not `mkdir(null)` / `path.dirname(null)`). `releaseLease` returns absent. `readLease` returns null. Production callers always have configDir, so live leasing is not dropped. Pin: configDir write never touches home; no-configDir never writes home or cwd/`null`; `XCLAW_CONFIG_DIR` still wins when no configDir.
RAN: node --test test/ledger-lease-config-dir.test.mjs test/ledger-lease.test.mjs test/ledger-lease-chaos.test.mjs test/lease-metrics.test.mjs test/lease-heartbeat.test.mjs test/lease-metrics-chaos.test.mjs test/doctor-ledger-wired.test.mjs test/swarm-ledger-lease-gate.test.mjs test/lease-backend-mock-redis.test.mjs test/ledger-lease-redis-skeleton.test.mjs → # tests 20 # pass 20 # fail 0 # duration_ms 248.092535; npm test (hermetic) → # tests 5221 # pass 5221 # fail 0 # duration_ms 84962.419513
UNVERIFIED: GitHub `ci` on this SHA; live two-instance configDir isolation on a running gateway with ledgerLease on; live acquire/renew under a non-home configDir.
NEXT: remaining non-S3 evolution gap from live source. Do not invert default-path durability. Do not rebuild S3 listed callers. Do not mint persistRun:true on voice, TUI stream body, Discord `/ask`, Slack, email, voice TUI, or voice listen. Do not auto-promote HTTP POST /agent/run. Do not stamp `runs resume --gateway` before GET `/health`.

## 2026-09-01 — 3.521.0 Last drain follows paths.configDir

STATUS: green (local hermetic pin)
DISCOVERED: JSON.parse / ENOENT / doctorHorizon({}) hunt exhausted (class 56/57). Load-await-write token-store remainder exhausted 3.500–3.504. Gateway request-path unbounded fetch exhausted 3.505.0. PagerDuty webhook history closed 3.506.0. Pairing store closed 3.507.0. Sessions persist closed 3.508.0. Usage-tracker ledger closed 3.509.0. Compaction offload closed 3.510.0. Telegram writer lock closed 3.511.0. Gateway supervised closed 3.512.0. Automations store closed 3.513.0. Control-plane sqlite closed 3.514.0. MITM confdir closed 3.515.0. Truth/sense null-confdir leftover closed 3.516.0. Cost governor closed 3.517.0. Account links closed 3.518.0. Swarm ledger closed 3.519.0. Ledger lease closed 3.520.0. Named remainder from source: `src/gateway/last-drain.mjs` `lastDrainPath()` resolved `~/.xclaw/last-drain.json` from `os.homedir()` while production stop writers (`recordLastDrain(drain, { cfg })` at stop-route / ws-stop-control / sse-stop-control) already had cfg in scope. `loadConfig()` stamps `paths.configDir` unconditionally, so the resolver still homed via the leftover `os.homedir()` fallback. Two instances on one host with different `paths.configDir` shared one last-drain.json; the suite wrote the operator's real `~/.xclaw`. Same class as v3.297.0 `alert-state.json` and v3.520.0 `swarm-ledger.lease`. Honour existing `XCLAW_CONFIG_DIR`. Do not honour `XCLAW_STATE_DIR`. No new env. Closed siblings (do not re-fix): ledger-lease (3.520.0), swarm-ledger (3.519.0), account-links (3.518.0), cost-governor (3.517.0), truth/sense null-confdir leftover (3.516.0), MITM confdir (3.515.0), control-plane (3.514.0), automations (3.513.0), gateway supervised (3.512.0), telegram lock (3.511.0), compaction (3.510.0), usage-tracker (3.509.0), sessions/persist (3.508.0), pairing-store (3.507.0), pagerduty-webhooks (3.506.0), alerts.mjs, cron/eval-job, cron/doctor-job, cron/live-e2e-job, model-stats.mjs, transcript, auth siblings. `cua-retry-metrics.mjs` `metricsPath()` is NOT this class (no cfg at live callers). `memoryIndexFile` / `agentStoreFile` have no production writer. Fabric `fabricDir()` / `fabricRoot()` take NO cfg — not this class. `ledger-lease-redis.mjs` is a Redis adapter skeleton — not this slice.
BUILT: `lastDrainPath(cfg)` honours `paths.configDir` then `XCLAW_CONFIG_DIR` then null. No home fallback. `recordLastDrain` no-ops a null path (in-memory `last` still set; do not `mkdir(null)` / `path.dirname(null)`). `loadLastDrain` returns in-memory last or null. Explicit `extra.path` still wins. Production writers always have configDir, so live persist is not dropped. Doctor threads cfg into `getLastDrain(cfg)` so a cross-process `xclaw doctor` still loads disk under configDir. Pin: configDir write never touches home; no-configDir never writes home or cwd/`null`; `XCLAW_CONFIG_DIR` still wins when no configDir; extra.path still writes.
RAN: node --test test/last-drain-config-dir.test.mjs test/last-drain.test.mjs test/last-drain-channel-complete.test.mjs test/last-drain-swarm.test.mjs test/doctor-last-drain-auth.test.mjs test/ws-stop-last-drain.test.mjs test/sse-stop-control.test.mjs test/stop-kill-switch.test.mjs test/smoke-last-drain.test.mjs test/autonomy-stop-channel.test.mjs test/autonomy-offline-gate.test.mjs → # tests 23 # pass 23 # fail 0 # duration_ms 245.614051; npm test (hermetic) → # tests 5227 # pass 5227 # fail 0 # duration_ms 78881.156056
UNVERIFIED: GitHub `ci` on this SHA; live two-instance configDir isolation on a running gateway; live POST /stop then `xclaw doctor` under a non-home configDir.
NEXT: remaining non-S3 evolution gap from live source. Do not invert default-path durability. Do not rebuild S3 listed callers. Do not mint persistRun:true on voice, TUI stream body, Discord `/ask`, Slack, email, voice TUI, or voice listen. Do not auto-promote HTTP POST /agent/run. Do not stamp `runs resume --gateway` before GET `/health`.

## 2026-09-01 — 3.522.0 MCP OAuth store follows paths.configDir

STATUS: green (local hermetic pin)
DISCOVERED: JSON.parse / ENOENT / doctorHorizon({}) hunt exhausted (class 56/57). Load-await-write token-store remainder exhausted 3.500–3.504. Gateway request-path unbounded fetch exhausted 3.505.0. PagerDuty webhook history closed 3.506.0. Pairing store closed 3.507.0. Sessions persist closed 3.508.0. Usage-tracker ledger closed 3.509.0. Compaction offload closed 3.510.0. Telegram writer lock closed 3.511.0. Gateway supervised closed 3.512.0. Automations store closed 3.513.0. Control-plane sqlite closed 3.514.0. MITM confdir closed 3.515.0. Truth/sense null-confdir leftover closed 3.516.0. Cost governor closed 3.517.0. Account links closed 3.518.0. Swarm ledger closed 3.519.0. Ledger lease closed 3.520.0. Last drain closed 3.521.0. Named remainder from source: `src/mcp/oauth.mjs` `storePath()` resolved `~/.xclaw/mcp-oauth.json` from `os.homedir()` while production writers (`storeMcpGrant(cfg)` / `dropMcpGrant(cfg)` at gateway/routes/mcp.mjs completeOAuthFlow / DELETE /mcp/oauth) already had cfg in scope. `loadConfig()` stamps `paths.configDir` unconditionally, so the resolver still homed via the leftover `os.homedir()` fallback. Two instances on one host with different `paths.configDir` shared one mcp-oauth.json; the suite wrote the operator's real `~/.xclaw`. Same class as v3.297.0 `alert-state.json` and v3.521.0 `last-drain.json`. Honour existing `XCLAW_CONFIG_DIR`. Do not honour `XCLAW_STATE_DIR`. No new env. Closed siblings (do not re-fix): last-drain (3.521.0), ledger-lease (3.520.0), swarm-ledger (3.519.0), account-links (3.518.0), cost-governor (3.517.0), truth/sense null-confdir leftover (3.516.0), MITM confdir (3.515.0), control-plane (3.514.0), automations (3.513.0), gateway supervised (3.512.0), telegram lock (3.511.0), compaction (3.510.0), usage-tracker (3.509.0), sessions/persist (3.508.0), pairing-store (3.507.0), pagerduty-webhooks (3.506.0), alerts.mjs, cron/eval-job, cron/doctor-job, cron/live-e2e-job, model-stats.mjs, transcript, auth siblings. `cua-retry-metrics.mjs` `metricsPath()` is NOT this class (no cfg at live callers). `memoryIndexFile` / `agentStoreFile` have no production writer. Fabric `fabricDir()` / `fabricRoot()` take NO cfg — not this class. `oauth-pending.mjs` has no production writer of `createPending`. `connected/token-store.mjs` is a sibling — not this slice.
BUILT: `storePath(cfg)` honours `paths.configDir` then `XCLAW_CONFIG_DIR` then null. No home fallback. `saveMcpOAuthStore` no-ops a null path (do not `mkdir(null)` / `path.dirname(null)`). `loadMcpOAuthStore` returns `{}`. Production writers always have configDir, so live persist is not dropped. Keep settleAfterMcpRefresh. Keep 0o600. Pin: configDir write never touches home; no-configDir never writes home or cwd/`null`; `XCLAW_CONFIG_DIR` still wins when no configDir.
RAN: node --test test/mcp-oauth-config-dir.test.mjs test/mcp-oauth.test.mjs test/mcp-oauth-settle.test.mjs test/mcp-oauth-flow-ttl.test.mjs test/mcp-oauth-callback-xss.test.mjs → # tests 21 # pass 21 # fail 0 # duration_ms 2155.618762; npm test (hermetic) → # tests 5232 # pass 5232 # fail 0 # duration_ms 77432.506603
UNVERIFIED: GitHub `ci` on this SHA; live two-instance configDir isolation on a running gateway; live MCP OAuth complete under a non-home configDir.
NEXT: remaining non-S3 evolution gap from live source. `connected/token-store.mjs` is a sibling. Do not invert default-path durability. Do not rebuild S3 listed callers. Do not mint persistRun:true on voice, TUI stream body, Discord `/ask`, Slack, email, voice TUI, or voice listen. Do not auto-promote HTTP POST /agent/run. Do not stamp `runs resume --gateway` before GET `/health`.

## 2026-09-01 — 3.523.0 Connected token store follows paths.configDir

STATUS: green (local hermetic pin)
DISCOVERED: JSON.parse / ENOENT / doctorHorizon({}) hunt exhausted (class 56/57). Load-await-write token-store remainder exhausted 3.500–3.504. Gateway request-path unbounded fetch exhausted 3.505.0. PagerDuty webhook history closed 3.506.0. Pairing store closed 3.507.0. Sessions persist closed 3.508.0. Usage-tracker ledger closed 3.509.0. Compaction offload closed 3.510.0. Telegram writer lock closed 3.511.0. Gateway supervised closed 3.512.0. Automations store closed 3.513.0. Control-plane sqlite closed 3.514.0. MITM confdir closed 3.515.0. Truth/sense null-confdir leftover closed 3.516.0. Cost governor closed 3.517.0. Account links closed 3.518.0. Swarm ledger closed 3.519.0. Ledger lease closed 3.520.0. Last drain closed 3.521.0. MCP oauth closed 3.522.0. Named remainder from source: `src/connected/token-store.mjs` `storePath()` resolved `~/.xclaw/connected-tokens.json` from `os.homedir()` while production writers (`setAppToken(cfg)` at oauth-callback / oauth-login) already had cfg in scope. `loadConfig()` stamps `paths.configDir` unconditionally, so the resolver still homed via the leftover `os.homedir()` fallback. Two instances on one host with different `paths.configDir` shared one connected-tokens.json; the suite wrote the operator's real `~/.xclaw`. Same class as v3.297.0 `alert-state.json` and v3.522.0 `mcp-oauth.json`. Honour existing `XCLAW_CONFIG_DIR`. Do not honour `XCLAW_STATE_DIR`. No new env. Closed siblings (do not re-fix): MCP oauth (3.522.0), last-drain (3.521.0), ledger-lease (3.520.0), swarm-ledger (3.519.0), account-links (3.518.0), cost-governor (3.517.0), truth/sense null-confdir leftover (3.516.0), MITM confdir (3.515.0), control-plane (3.514.0), automations (3.513.0), gateway supervised (3.512.0), telegram lock (3.511.0), compaction (3.510.0), usage-tracker (3.509.0), sessions/persist (3.508.0), pairing-store (3.507.0), pagerduty-webhooks (3.506.0), alerts.mjs, cron/eval-job, cron/doctor-job, cron/live-e2e-job, model-stats.mjs, transcript, auth siblings. `cua-retry-metrics.mjs` `metricsPath()` is NOT this class (no cfg at live callers). `memoryIndexFile` / `agentStoreFile` have no production writer. Fabric `fabricDir()` / `fabricRoot()` take NO cfg — not this class. `oauth-pending.mjs` has no production writer of `createPending`. `ledger-lease-redis.mjs` is a Redis adapter skeleton — not this slice.
BUILT: `storePath(cfg)` honours `paths.configDir` then `XCLAW_CONFIG_DIR` then null. No home fallback. `saveTokens` no-ops a null path (do not `mkdir(null)` / `path.dirname(null)`). `loadTokens` returns `{ version: 1, apps: {} }`. Production writers always have configDir, so live persist is not dropped. Keep settleAfterAppRefresh. Keep AES-256-GCM. Keep 0o600. Pin: configDir write never touches home; no-configDir never writes home or cwd/`null`; `XCLAW_CONFIG_DIR` still wins when no configDir.
RAN: node --test test/token-store-config-dir.test.mjs test/token-refresh.test.mjs test/token-refresh-mock.test.mjs test/oauth-token-refresh.test.mjs → # tests 16 # pass 16 # fail 0 # duration_ms 124.945091; npm test (hermetic) → # tests 5237 # pass 5237 # fail 0 # duration_ms 78077.156311
UNVERIFIED: GitHub `ci` on this SHA; live two-instance configDir isolation on a running gateway; live connected-app OAuth complete under a non-home configDir.
NEXT: remaining non-S3 evolution gap from live source. Do not invert default-path durability. Do not rebuild S3 listed callers. Do not mint persistRun:true on voice, TUI stream body, Discord `/ask`, Slack, email, voice TUI, or voice listen. Do not auto-promote HTTP POST /agent/run. Do not stamp `runs resume --gateway` before GET `/health`.

## 2026-09-01 — 3.524.0 Connected vault follows paths.configDir

STATUS: green (local hermetic pin)
DISCOVERED: JSON.parse / ENOENT / doctorHorizon({}) hunt exhausted (class 56/57). Load-await-write token-store remainder exhausted 3.500–3.504. Gateway request-path unbounded fetch exhausted 3.505.0. PagerDuty webhook history closed 3.506.0. Pairing store closed 3.507.0. Sessions persist closed 3.508.0. Usage-tracker ledger closed 3.509.0. Compaction offload closed 3.510.0. Telegram writer lock closed 3.511.0. Gateway supervised closed 3.512.0. Automations store closed 3.513.0. Control-plane sqlite closed 3.514.0. MITM confdir closed 3.515.0. Truth/sense null-confdir leftover closed 3.516.0. Cost governor closed 3.517.0. Account links closed 3.518.0. Swarm ledger closed 3.519.0. Ledger lease closed 3.520.0. Last drain closed 3.521.0. MCP oauth closed 3.522.0. Connected token store closed 3.523.0. Named remainder from source: `src/connected/vault.mjs` `vaultRoot()` resolved `~/.xclaw/vault` from `os.homedir()` while production writers (`vaultMergeIntoAccount(cfg)` at account-links / `vaultDeleteApp(cfg)` at auth-legacy-cli) already had cfg in scope. `loadConfig()` stamps `paths.configDir` unconditionally, so the resolver still homed via the leftover `os.homedir()` fallback. Two instances on one host with different `paths.configDir` shared one vault; the suite wrote the operator's real `~/.xclaw`. Same class as v3.297.0 `alert-state.json` and v3.523.0 `connected-tokens.json`. Honour existing `XCLAW_CONFIG_DIR`. Do not honour `XCLAW_STATE_DIR`. No new env. Closed siblings (do not re-fix): connected token store (3.523.0), MCP oauth (3.522.0), last-drain (3.521.0), ledger-lease (3.520.0), swarm-ledger (3.519.0), account-links (3.518.0), cost-governor (3.517.0), truth/sense null-confdir leftover (3.516.0), MITM confdir (3.515.0), control-plane (3.514.0), automations (3.513.0), gateway supervised (3.512.0), telegram lock (3.511.0), compaction (3.510.0), usage-tracker (3.509.0), sessions/persist (3.508.0), pairing-store (3.507.0), pagerduty-webhooks (3.506.0), alerts.mjs, cron/eval-job, cron/doctor-job, cron/live-e2e-job, model-stats.mjs, transcript, auth siblings. `cua-retry-metrics.mjs` `metricsPath()` is NOT this class (no cfg at live callers). `memoryIndexFile` / `agentStoreFile` have no production writer. Fabric `fabricDir()` / `fabricRoot()` take NO cfg — not this class. `oauth-pending.mjs` has no production writer of `createPending`. `ledger-lease-redis.mjs` is a Redis adapter skeleton — not this slice. `tokens/auth-refresh-status.mjs` is a leftover sibling — not this slice.
BUILT: `vaultRoot(cfg)` honours `paths.configDir` then `XCLAW_CONFIG_DIR` then null. No home fallback. `vaultSave` no-ops a null path (do not `mkdir(null)` / `path.dirname(null)` / rename `"null.bak-*"`). `vaultLoad` returns `{ version: 1, apps: {}, userId }`. Production writers always have configDir, so live persist is not dropped. Keep AES-256-GCM. Keep 0o600. Pin: configDir write never touches home; no-configDir never writes home or cwd/`null`; `XCLAW_CONFIG_DIR` still wins when no configDir.
...
## 2026-09-01 — 3.531.0 Eval history follows paths.configDir

LOCKED: `src/eval/history.mjs` `historyPath` still homed while production writer `appendEvalHistory(cfg)` at eval/runner.mjs:210 and readers `listEvalHistory(cfg)` at gateway/dashboard, gateway/routes/eval-queue, eval/scoreboard, eval/spend already had cfg. Same class as v3.297.0 / v3.530.0.

DISCOVERED: leftover `os.homedir()` fallback after `cfg?.paths?.configDir`. Two instances on one host shared one `eval-history.jsonl`; the suite wrote the operator's real `~/.xclaw`. Existing tests already pass `{ paths: { configDir } }`. Did not rewrite line shape. cron/logs REJECTED (reader-home-by-design). run-store REJECTED (persistRun durability).

SHIPPED: honour `paths.configDir` then `XCLAW_CONFIG_DIR` then null. No home fallback. Do not honour `XCLAW_STATE_DIR`. No new env. `appendEvalHistory` still returns the in-memory line without persisting (do not `mkdir(null)`). `listEvalHistory` returns `[]`. Production writers always have configDir, so live persist is not dropped. Removed `os` import. Exported `evalHistoryPath`; `function historyPath` wraps it.
...
## 2026-09-02 — 3.544.0 Swarm receipts follow paths.configDir

LOCKED: `src/agents/swarm-receipt.mjs` `swarmsRoot` still homed while production writer `writeNodeReceipt(cfg)` via `attachNodeReceipt(cfg)` at agents/swarm-run.mjs:498/727/755/819 already had cfg. Same class as v3.297.0 / v3.543.0.

DISCOVERED: leftover `os.homedir()` fallback after `cfg.paths?.configDir`. Two instances on one host shared one `swarms/runs/<id>/receipts`; the suite wrote the operator's real `~/.xclaw/swarms`. Existing receipt-schema tests already pass `{ paths: { configDir } }`. Removed `os` import (only used by the leftover home fallback). Did not rewrite receipt JSON shape / RECEIPT_SCHEMA_V1 / inferEffects / evaluateReceiptPolicy. cron/logs REJECTED (reader-home-by-design). run-store REJECTED (persistRun durability). Did not also fix swarm-merge.

SHIPPED: honour `paths.configDir` then `XCLAW_CONFIG_DIR` then null. No home fallback. Do not honour `XCLAW_STATE_DIR`. No new env. `writeNodeReceipt` still returns `{ ok: true, path: null, receipt }` without persisting (do not `mkdir(null)`). `readNodeReceipt` returns `null`. `listNodeReceipts` returns `[]`. Production writers always have configDir, so live persist is not dropped. Exported `swarmReceiptsRoot`.
...
## 2026-09-02 — 3.548.0 Skill-loop metrics follow paths.configDir

LOCKED: `src/skills/loop.mjs` `metricsPath` still homed while production writers `recordSkillLoopMetric(cfg)` via `runSkillAB(cfg)` at skills/loop.mjs:107/141 and `bin/xclaw.mjs:2317` after `loadConfig()` already had cfg. Same class as v3.297.0 / v3.547.0.

DISCOVERED: leftover `os.homedir()` fallback after `cfg.paths?.configDir`. Two instances on one host shared one `skill-loop-metrics.jsonl`; the suite wrote the operator's real `~/.xclaw`. Existing skill-ab tests only import `computeSkillDelta` and do not write. Removed `os` import (only used by the leftover home fallback). Did not rewrite computeSkillDelta / promoteAndInstall / runSkillAB / runSkillABBatch. cron/logs REJECTED (reader-home-by-design). run-store REJECTED (persistRun durability). Did not also fix durable-jobs / tui / cold-start-persist.

SHIPPED: honour `paths.configDir` then `XCLAW_CONFIG_DIR` then null. No home fallback. Do not honour `XCLAW_STATE_DIR`. No new env. `recordSkillLoopMetric` still returns `null` without persisting (do not `mkdir(null)`). `readSkillLoopMetrics` returns `[]`. Production writers always have configDir, so live persist is not dropped. Exported `metricsPath`.
...
## 2026-09-02 — 3.552.0 Cookie rotation follows paths.configDir

LOCKED: `src/auth/cookie-rotation.mjs` `rotationPaths` still homed while production writers `rotateWebSession(cfg)` via `runAuthCli(cfg)` at bin/xclaw.mjs:49-53 after `loadConfig()`, auth-cli.mjs:123 already had cfg. Same class as v3.297.0 / v3.551.0.

DISCOVERED: leftover `os.homedir()` fallback after `cfg.paths?.configDir`. Two instances on one host shared one `cookie-rotation.json`; the suite wrote the operator's real `~/.xclaw`. Existing cookie-rotation.test.mjs already passes `{ paths: { configDir: dir } }`. KEEP `cfg.auth?.web?.rotationStatePath` / `previousSessionPath`. KEEP `XCLAW_COOKIE_ROTATION` as strategy env (not a path). Removed `os` import (only used by the leftover home fallback). Did not rewrite cookieFingerprint / evaluateRotation / recordSessionUse / bindAfterImport / gateWebSession / listRotationStrategies / ROTATION_STRATEGIES. cron/logs REJECTED (reader-home-by-design). run-store REJECTED (persistRun durability). cold-start-persist REJECTED (existing pin asserts default path under .xclaw). Did not also fix fingerprint-rotation / webauthn / key-rotation / jwks.

SHIPPED: honour `cfg.auth?.web?.rotationStatePath` / `previousSessionPath` then `paths.configDir` then `XCLAW_CONFIG_DIR` then null. No home fallback. Do not honour `XCLAW_STATE_DIR`. No new env. `writeState` still no-ops without persisting (do not `mkdir(null)`). `readState` returns the empty default. `rotateWebSession` skips archive when `previousPath` is null. Production writers always have configDir, so live persist is not dropped.
...
## 2026-09-02 — 3.578.0 Hooks Add drops leftover placeholder so filled body is not muted

LOCKED: Hooks #/hooks Add with empty command left class `log placeholder` on filled hkOut. Live 2026-09-02 pid 2798540 (version 3.562.0) Control #/hooks Add filled "command required" (16 chars); CDP reported cls `log placeholder`. CSS mutes and italicizes. Same leftover as Alerts PagerDuty 3.577.0, Hooks pane. Empty-state "no output yet" before Add is correct. Found as a user on Display :10. Homedir JSON store-writer class remains EXHAUSTED at 3.560.0. Do not reopen 3.577.0 Alerts PagerDuty. Do not POST /hooks/commands.

DISCOVERED: btnHkAdd set textContent only. HTML starts with class="log placeholder" and "no output yet".

SHIPPED: btnHkAdd fill path classList.remove("placeholder") then body. Pin test/control-auto-refresh.test.mjs (btnHkAdd slice removes placeholder; empty-state HTML kept). One copy.
...
## 2026-09-02 — 3.581.0 Images Generate drops leftover placeholder so filled body is not muted

LOCKED: Images #/media Generate with empty prompt left class `log placeholder` on filled mediaOut. Live 2026-09-02 pid 2798540 (version 3.562.0) Control #/media Generate filled "enter a prompt" (14 chars); CDP reported cls `log placeholder`. CSS mutes and italicizes. Same leftover as Subagents Spawn 3.580.0. Empty-state "no output yet" before Generate is correct. Found as a user on Display :10. Homedir JSON store-writer class remains EXHAUSTED at 3.560.0. Do not reopen 3.580.0 Subagents Spawn. Do not POST /media/jobs. Do not Generate with a prompt.

DISCOVERED: btnMediaGen set textContent only. HTML starts with class="log placeholder" and "no output yet".

SHIPPED: btnMediaGen fill path classList.remove("placeholder") then body. Pin test/control-auto-refresh.test.mjs (btnMediaGen slice removes placeholder; empty-state HTML kept). One copy.
