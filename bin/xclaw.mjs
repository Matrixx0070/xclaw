...
Enable with seats.enabled: true in config`);
  ...
  xclaw models auth ...`);
    ...
    // `merge` is the swarm merge-proposal CLI. This one merges a subagent's
    // git worktree and took an id of a different kind; sharing the label meant
    // the first case won and this was unreachable.
    case "merge-worktree": {
      const { getSubagent } = await import("../src/agents/spawn.mjs");
      const { mergeSubagentWorktree } = await import("../src/agents/worktree.mjs");
      const id = args[1];
      if (!id) { console.error("Usage: xclaw merge-worktree <subagentId> [--check] [--repo path]"); process.exit(1); }
