#!/usr/bin/env node
/** Idempotent: land syntheticG20Job + includeG20; also run apply-n12m-g19 */
...
if (!t.includes("horizon-g20-metrics")) {
  const anchors = [
...
if (!t.includes('id.includes("G20")')) {
  for (const g of ["G19", "G18", "G17", "G16", "G15", "G14"]) {
...
if (!t.includes("syntheticG20Job")) {
  ...
  for (const s of [
    ...
    if (t.includes(s) && !t.includes("syntheticG20Job,")) {
      t = t.replace(s, s + "\n  syntheticG20Job,");
      break;
    }
  }
...
if (!t.includes('jobs["a4-G20-cost-stop-resume"]')) {
  t = t.replace(
...
if (!t.includes('includeG20 ? ["a4-G20-cost-stop-resume"]')) {
  for (const prev of [
